...
float3 DecodeNormal(thread float3& normalTS, thread const float& normalIntensity)
{
    ...
    float3 _285 = InverseNormalY(param);
    normalTS = _285;
    float3 param_1 = normalTS;
    float param_2 = normalIntensity;
    float3 _290 = NormalIntensity(param_1, param_2);
    normalTS = _290;
...
float3 SamplerEncodedPanoramic(thread const texture2d<float> panoramic, thread const sampler panoramicSmplr, thread const float3& dir, thread const float& rotation, thread const float& lod)
{
    ...
    float2 _1298 = GetPanoramicTexCoordsFromDir(param, param_1);
    float2 uv = _1298;
    float2 param_2 = uv;
    float param_3 = lod;
...
float3 Diffuse_OrenNayar(thread const SurfaceParams& S, thread const LightParams& L)
{
    ...
    if (Cosri >= 0.0)
    {
        _486 = 1.0 / fast::max(ndl, ndv);
    }
    else
    {
        _486 = 1.0;
    }
    float C2 = (((0.449999988079071044921875 * s2) / (s2 + 0.0900000035762786865234375)) * Cosri) * _486;
...
static inline __attribute__((always_inline))
float3 EmissiveTerm(thread const SurfaceParams& S)
{
    return S.emissive;
}
...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> _AlbedoTexture [[texture(0)]], texture2d<float> _MRAOTexture [[texture(1)]], texture2d<float> _NormalTexture [[texture(2)]], texture2d<float> _EmissiveTexture [[texture(3)]], texture2d<float> _AmbientTexture [[texture(4)]], sampler _AlbedoTextureSmplr [[sampler(0)]], sampler _MRAOTextureSmplr [[sampler(1)]], sampler _NormalTextureSmplr [[sampler(2)]], sampler _EmissiveTextureSmplr [[sampler(3)]], sampler _AmbientTextureSmplr [[sampler(4)]])
{
    main0_out out = {};
    float2 uv0 = in.v_uv0;
    ...
    float3 _2229 = DecodeNormal(param_8, param_9);
    clearCoatNormal = _2229;
    normal = _2229;
    clearCoatNormal = TBN * clearCoatNormal;
    normal = clearCoatNormal;
    float4 t_EmissiveTex = _EmissiveTexture.sample(_EmissiveTextureSmplr, uv0);
    float3 param_10 = buffer._EmissiveColor.xyz;
    float3 param_11 = t_EmissiveTex.xyz;
    float3 emissive = (GammaToLinear(param_10) * GammaToLinear(param_11)) * buffer._EmissiveIntensity;
    float param_12 = envInt;
    float param_13 = envRot;
    float3 param_14 = albedo;
    float param_15 = opacity;
    float param_16 = cutoff;
    float3 param_17 = normal;
    float3 param_18 = clearCoatNormal;
    float param_19 = metallic;
    float param_20 = roughness;
    float param_21 = ao;
    float subsurface;
    float param_22 = subsurface;
    float3 subsurfaceCol;
    float3 param_23 = subsurfaceCol;
    float3 subsurfaceColMultiply;
    float3 param_24 = subsurfaceColMultiply;
    float ior;
    float param_25 = ior;
    float transmittance;
    float param_26 = transmittance;
    float transmittanceColorAtDistance;
    float param_27 = transmittanceColorAtDistance;
    float thin;
    float param_28 = thin;
    float clearCoat;
    float param_29 = clearCoat;
    float clearCoatRoughness;
    float param_30 = clearCoatRoughness;
    float3 param_31 = emissive;
    float anisotropic;
    float param_32 = anisotropic;
    float anisotropicRotate;
    float param_33 = anisotropicRotate;
    float rampID;
    float param_34 = rampID;
    float rim;
    float param_35 = rim;
    float3 rimCol;
    float3 param_36 = rimCol;
    float3 ambient;
    float3 param_37 = ambient;
    float3 matcap;
    float3 param_38 = matcap;
    float smoothFactor;
    float param_39 = smoothFactor;
    float4 finalColor = MainEntry(_AmbientTexture, _AmbientTextureSmplr, param_12, param_13, param_14, param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23, param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32, param_33, param_34, param_35, param_36, param_37, param_38, param_39, in.v_posWS, in.v_nDirWS, in.v_tDirWS, in.v_bDirWS, buffer.u_WorldSpaceCameraPos, buffer.u_DirLightsEnabled, buffer.u_DirLightNum, buffer.u_DirLightsDirection, buffer.u_DirLightsColor, buffer.u_DirLightsIntensity, buffer.u_PointLightsEnabled, buffer.u_PointLightNum, buffer.u_PointLightsPosition, buffer.u_PointLightsColor, buffer.u_PointLightsIntensity, buffer.u_PointLightsAttenRangeInv, buffer.u_SpotLightsEnabled, buffer.u_SpotLightNum, buffer.u_SpotLightsPosition, buffer.u_SpotLightsColor, buffer.u_SpotLightsIntensity, buffer.u_SpotLightsAttenRangeInv, buffer.u_SpotLightsDirection, buffer.u_SpotLightsOuterAngleCos, buffer.u_SpotLightsInnerAngleCos);
    ...
    float2 param_41 = screen_coord;
    out.glResult = ApplyBlendMode(param_40, param_41);
    return out;
}
